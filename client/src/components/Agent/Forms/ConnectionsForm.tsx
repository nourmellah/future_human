...
export default function ConnectionsForm({ initial = [], onChange }: ConnectionsFormProps) {
  ...
  // Single helper to update + notify parent
  function updateRows(next: ConnectionItem[]) {
    setRows(next);
    initialRef.current = next;
    onChange?.(next);
  }

  // === Actions (no API calls here) ===
  function openCatalog() { setCatalogOpen(true); }
  function closeCatalog() { setCatalogOpen(false); }
  ...
  function closeConfig() { setConfigOpen(false); setConfigTarget(null); }

  function saveConfig(values: Record<string, string>) {
    if (!configTarget) return;
    updateRows(
      rows.map((r) => (String(r.id) === configTarget.id ? { ...r, config: values } : r))
    );
    closeConfig();
  }
